...
**The empty-space skip — built (`feat/empty-skip`), and the line closes as a comprehensive null.** On an `Empty` rope we jumped past the *whole* empty cell (`cell_size(level-1)` voxels) instead of just the 8-voxel neighbour brick, again validated **byte-identical** to the baseline. The realized node-fetch count was **unchanged — bit-for-bit identical to the leaf-only version** (sierpinski 0.97×, caves 1.10×, dust 0.94×, wire 1.03× vs baseline). Two reasons, one of them fatal to the whole idea. (i) **The empties are fine-grained:** for these fixtures the absent child surfaces at a *low* level (the brick level, cell size ≈ 8), so "skip the whole empty cell" almost never skips more than the neighbour brick it was already skipping — there is no coarse empty extent to merge. (ii) **The dominant cost is re-descend-from-root, and the jump distance cannot touch it:** the rope traversal re-descends from the root once per occupied leaf visited, and that root walk dominates the fetch count regardless of *where* the empty advance lands. This is the decisive point, because it inverts the premise: the rope's stackless re-descent **is itself the cost**, and the baseline already avoids it by ascending only to the *common ancestor* — a single scalar-stack pop on the register kernel. The stackless rope re-dives `k` levels from root; the stack-based baseline pops one or two. **For a hierarchy with a cheap (register-resident) stack, the rope is strictly the worse of the two — block-walking is a downgrade here, not a win.** Across all four sub-experiments — leaf ropes (null), early-skip (null/regressive), empty-classification (theoretically-skippable but unmergeable), empty-cell jump (null) — the verdict is identical and the line is now closed. The one variant left unbuilt is **full node-level ropes** (neighbour pointers on every node, never re-descending from root); the synthesis bounds even a perfect implementation to marginal — our ascend is already cheap, we are latency-bound on the cross-block miss regardless (§ Aila & Laine), and it would forfeit the cheap incremental edits — so it was not built.

**Bottom-up caching (read accessors) — the read-side twin of the block-walking null, declined on principled grounds.** The OpenVDB `ValueAccessor` pattern caches the leaf→root node chain from the previous access and, on the next query, re-enters *bottom-up* (test the cached leaf, then its parent, …, stopping at the first ancestor that still encloses the coordinate) instead of re-descending top-down from the root. Evaluated by a four-lens pass (code-truth, ceiling-from-data, literature, adversarial steelman); **not built**, because for our two product surfaces — ray traversal and incremental edits — it is a null on the same structural grounds that closed block-walking, and the cheapest falsifier was the existing `measure` data rather than an implementation. (i) **For ray traversal we already perform the within-ray form of it.** The HDDA descends from root exactly once per ray (`layout.rs` `walk` entry / `traversal.wgsl` root frame) and on every mid-march exit ascends only to the *common ancestor* via the register/recursion stack — a scalar-stack pop, no node read on the pop itself. That *is* the accessor's bottom-up re-entry, but with an always-exact stack rather than a heuristic cache that can miss; the just-closed ropes did the opposite (re-descend `k` levels from root) and lost to exactly this. (ii) **The only cross-ray-shareable work is the one-time root→first-leaf descent (`k = 3` fetches at 512³), and it is both tiny and cache-warm.** Measured descent fraction of total cell-steps: dust 1.9 %, wire 6.5 %, caves 12.7 %, sierpinski 18.9 % → absolute cross-ray ceilings of **1.02× / 1.07× / 1.15× / 1.23×** even if the entire shared descent were free. The traversal-killers (dust, wire) have the *smallest* ceilings — their long marches drown the 3-fetch descent — and those `k` nodes are the hottest cache lines in the structure (the root is read by every ray, effectively L1-pinned), so an accessor swaps an L1 hit for a bounds-check on the same node: a sliver of a sub-1.2× ceiling, below the 2–6× thermal noise floor. On GPU it is worse than redundant — warp convergence already captures the hot-root reuse, and a tile-shared accessor would reintroduce shared-memory traffic that fights the register-resident design the ~1.8× kernel win was built on. As ever, it attacks the warm/ALU part; the latency bottleneck is the cold cross-block *leaf* miss it cannot reach. (iii) **The edit path gives it nothing:** `set_voxel` does not descend the tree — it binary-searches the flat sorted `codes` array and splices — so there is no path to cache. **The one genuinely live case is deferred, not declined:** a future *spatially-coherent point-query API* — surface meshing (marching cubes / dual contouring), brush/stamp sculpt reads, swept-volume collision sampling, CSG boolean tree-walks, flood-fill / SDF sweeps, stencil neighbour-gathers. These are independent, stateless, spatially-local queries with **no recursion stack to ride**, and today `SparseTree::is_occupied` re-descends all `k` levels from the coarse root on every call (it has no production hot consumer — it is build/test-only). For a coherent query stream the accessor is the *correct* tool there and uniquely so, plausibly ~2–4× at `k = 4`, matching OpenVDB's reported numbers — but the gain is contingent on such an API existing, which it does not yet. So: do not build it for traversal or edits (confirmed null); build it *first* for whatever coherent host-side query/sampling/meshing/CSG subsystem is added next, where it stops being a relayout gimmick and becomes the right primitive. It is purely additive and oracle-safe — `accessor.get(c)` returns bit-identical results to `is_occupied(c)`, verifiable by the same exact-equality discipline used everywhere else.

### The GPU counter capture — the floor is the kernel's *own stack*, not the cold leaf miss

This is the capstone, and it overturns the premise the entire optimization program reasoned from. Every layout/work lever above came back null, and the recurring explanation — *"latency-bound on the cold cross-block **leaf** miss"* (§ Aila & Laine) — was never actually measured. It was inferred from the anisotropy correlation and a stack of falsified experiments; the only shipped instrumentation (`TIMESTAMP_QUERY`) brackets a pass begin→end and is blind to *why* the kernel is slow. So we built the instrument: a `voxel capture` harness (loops the worst-orientation kernel as a stable profiler target) and a programmatic `.gputrace` writer (`MTLCaptureManager` via `objc2`, exposed as `voxel capture --gputrace` / `make gputrace` — captures the system-default Metal device, which on a single-GPU Apple-silicon Mac is the same device `wgpu` uses), and took an Xcode GPU counter capture of the `hdda pass` (the `traverse_ray` kernel, `traversal.wgsl:211`) on **`dust` 2048³ at the worst-anisotropy orientation**.

**The counters refute the cold-leaf-miss premise outright.** Read on the kernel during the dispatch:

| counter | value | reading |
|---|---|---|
| L1 read bandwidth, **by source** | **Stack 99.6 GiB/s (78 %)**, Buffer (voxel data) 26 GiB/s (20 %), Register 2.7 | the dominant L1 traffic is the kernel's **own stack**, *not* the structure |
| Device / GPU bandwidth | **11.7 GiB/s** (4.9 R / 6.8 W) | trivial vs ~200+ peak — **not** memory-bandwidth bound |
| L1 residency | **Register 91.95 %**, Buffer 7.29 %, Stack 0 % (read-through), Total 99.24 % | registers consume the L1/register SRAM; only 7 % left for buffer caching |
| Kernel occupancy | **17.87 %** | starved — too few warps resident to hide any latency |
| Top limiter | **Instruction Throughput ~60 %** (nothing pegged at 100 %) | underutilized / latency-exposed, not saturated on any unit |
| Last-level cache | idle | the cold cross-block leaf miss we obsessed over is **not** happening |

**The per-line shader profile bullseyes it to two functions.** Cost by function: **`walker_step` 33.07 %** (`traversal.wgsl:155`), **`make_frame` 13.02 %** (`:136`), **`leaf_reaches` 10.70 %** (`:65`, of which its only buffer read `leaf_bounds[slot]` at `:66` is **0.38 %** — the function is pure slab-test ALU). The true voxel-data reads — `nodes[cur.node]` (`:257`) and `leaf_words[...]` (`:57`) — are **< 1 %** each. Inside `walker_step` every hot line is an `(*f).*` access — `(*f).cell.*`, `(*f).t_max.*`, `(*f).t_delta[..]` — i.e. loads/stores against the frame in memory, not registers.

**Root cause, located to the line.** The *active* frame is already register-resident (`var cur`, `:235`; `walker_step` takes `ptr<function, Frame>`, `:155`) — that is the banked ~1.8× register-kernel win. The residual is the **ancestor stack**: `var stack: array<Frame, 8>` (`:236`), **dynamically indexed by the runtime `sp`** — store `stack[sp] = cur` on descend (`:270`), loads `cur = stack[sp]` on ascend (`:251`, `:277`). A dynamically-indexed function-scope array is exactly what Naga/Metal cannot keep in registers, so `wgpu` lowers it to thread-local *private* ("stack") memory that is **read-through L1 at 0 % residency** — the precise capture signature. The `Frame` struct (`:93–104`) is **112 bytes** by WGSL std layout, of which ~40 is pure padding (the explicit `_pad:u32` at `:97`, plus the 4-byte tail Naga adds to each of the five 16-byte-aligned `vec3` members `origin/cell/step/t_max/t_delta`). So `array<Frame, 8>` is **896 B/thread** of private memory, pushed/popped once per descend/ascend (~37 push-pop pairs per `dust` ray at the worst orientation, against 143 node-fetches/ray). That traffic — plus Naga's spills of the live frame — is the 99.6 GiB/s, and the 896 B co-resident with the live frame is what pins register residency at 92 % and caps occupancy at 18 %. **We spend ~4× more bandwidth reading our own traversal stack than reading the voxel geometry.**

**Why this does not contradict Door B (verified against `feat/occupancy`'s diff).** Door B's slim-frame null (`dust` 1.02×) did *not* test the lever this capture motivates. Door B (1) shrank bytes only modestly and never scalarized the `vec3` padding; (2) **recomputed `dim`/`step`/`t_delta` on every hot-loop step**, adding ALU to the instruction-throughput path the capture shows is already the top limiter; and (3) **kept the dynamically-indexed `stack[sp]`** — so the read-through-private-memory access *pattern* that produces the 99.6 GiB/s was byte-for-byte unchanged. Door B therefore falsified *"shrink the per-frame bytes a little + recompute in the hot loop,"* **not** *"kill the dynamic-index read-through stack + drop the padding without recompute."* It had no GPU counters, so it could not see that the stack pattern, not the buffer reads, was the dominant L1 consumer — the by-source breakdown is genuinely new information.

**The honest ceiling — a floor shift, not the headline fix.** Even a perfect result lowers the *absolute floor* (Door-A class, ~1.8×); it does **not** flatten the **orientation anisotropy** (the ~9× swing that is the actual subject of §5). The hardware-free evidence bounds that swing as intrinsic cross-block-miss / divergence latency: `dust` cell-step swing is flat (~1.5×) while GPU swing is ~8.9× at r≈0.24 (§5.4/§5.8), and the cache/coherence excess was *unchanged-to-larger* when re-measured **on** the register kernel (`dust` 3.39→3.87×, §11 Door A). The capture refutes the cold-leaf premise for the **floor** (device BW trivial, LLC idle), but the **swing** remains latency-bound — occupancy cannot hide it.

**The lever, built and measured (`feat/kernel-stack-experiments`),** each gated on the **0/20000 byte-identical differential** and confirmed by *re-capturing the by-source L1 split*, not wall-clock alone (because the #1 risk is **re-spill** — a register ladder that Naga still spills just relabels Stack as Register with little BW change): **(1)** `array<Frame, 8>→6` (`:236`) — trim the 896 B footprint (true max depth is 6, mirror.rs:20); **(2)** scalarize `Frame` (`:93–104`) — drop `_pad` and replace the four `vec3` members with bare per-axis scalars, killing ~40 B of alignment padding with **zero recompute** (distinct from Door B); **(3)** eliminate the dynamic `stack[sp]` (`:251/:270/:277`) via a `switch(sp)` register ladder so the small stack register-promotes — collapses the 78 %-of-L1 read-through Stack source; **(4)** sweep `@workgroup_size` (hdda.wgsl:23, render.wgsl:24) once the freed SRAM raises occupancy off 18 %.

**Outcome — null-to-negative; the cap is *active-frame* register pressure, which footprint cannot touch.** EXP1 (`array<Frame,8>→6`) and EXP2 (scalarize, 112→76 B) were built, validated byte-identical (0/20000), and re-captured on `dust` 2048³. **L1 Register residency was unchanged (~92 %); occupancy stayed at ~18 %.** The shrink freed *private-memory stack* bytes — but those are read-through at 0 % L1 residency and were never what capped occupancy. The cap is the **active frame's register allocation** (~19 live values: node/level/dim + cell/step/t_max/t_delta · xyz + t_entry), which a stack/footprint change does not reduce. Worse, EXP2 made `walker_step` *more* expensive (33.07 %→**37.42 %**): three scalar loads replaced one `vec3` load on the instruction-throughput-limited kernel, and a bracketed A/B was flat-to-slightly-slower — so EXP2 was **reverted** (EXP1 kept as a harmless cleanup; the stack was genuinely oversized). This also forecloses **EXP3 without building it**: register-promoting the ancestor stack would *add* pressure on top of the 92 % residency → it re-spills (no change) or craters occupancy — a predicted net-negative. The only way to cut the active-frame register count is to *recompute* state rather than store it — which is **Door B, already null (1.02×)**. So the kernel-micro line closes: the floor is **register-pressure-capped occupancy (~18 %) + instruction-throughput-limited + latency-exposed**, intrinsic to a register-resident DDA whose live state is inherently register-heavy.

**The meta-lesson.** This vindicates measure-first twice over. The premise the whole program optimized against — the cold cross-block leaf miss — was never measured and is **wrong for the floor**; every layout lever was aimed at the 20 %. And the first lever the capture *seemed* to suggest (shrink the frame for occupancy) was Door B by another name — the five-lens adversarial pass flagged it, and the one genuinely-new variant (kill the dynamic-index stack + drop padding without recompute) was built, measured, and came back **null-to-negative**: the counter we re-captured to *prove* it proved the opposite (register residency unchanged), saving us from shipping a regression. The capture's lasting value is not a speedup but a *correct diagnosis* — the floor is **register-capped occupancy on inherently-stateful traversal**, not the cold leaf miss — and the path forward is to **budget around it** (§9.6 reprojection / adaptive internal-resolution) or move to **hardware RT / SER**, not more kernel surgery.

**The deepest read — ~90 % intra-warp *lockstep* divergence, and it is hardware-bound.** A further capture (Xcode Cost-Graph runtime statistics on the `hdda` kernel, `dust` 2048³ worst orientation) gives the true root: **Divergence ~90 %** — Instructions Executed *Total* ~48 M vs *Active* ~4.8 M, i.e. only **~10 % of issued instructions land on active lanes** — with **~100 live registers** and per-line ALU cost only ~3 %. The kernel runs at ~10 % SIMD efficiency; the *work* is cheap, but 90 % of every warp's lanes sit masked. The nature is **trip-count lockstep**, and the arithmetic is self-consistent: `traverse_ray` (`traversal.wgsl:211`) is a single `for` loop with per-ray `return` on hit, and level is carried as *data*, not control flow (§ the design's "all warp lanes execute the same instruction each iteration, only the level value differs"), so *branch* divergence was already engineered out. What remains is that `dust` rays run **~1 to ~143 iterations** of that one loop: a lane that strikes its voxel at step 1 sits exec-masked for ~142 iterations while its 31 simdgroup-mates keep issuing until the *longest* ray in the warp retires. `1/(trip-count spread) ≈ 10 %` **is** the measured 4.8 M / 48 M. The ~9× orientation anisotropy is this same quantity (the worst orientation maximises the neighbour-ray step-count spread).

A five-lens design pass (persistent threads, occupancy, branchless, literature+adversarial), grounded in Aila & Laine (HPG 2009/2010), forecloses **every** software lever expressible in WGSL/wgpu 29: **(a) Persistent threads** (atomic ray-counter work queue) fixes *inter*-warp load imbalance — but at our ray counts that is already absorbed by launch-grid oversubscription (the scheduler backfills retired warps for free, leaving only a sub-1 % tail), and Aila & Laine §3.3–3.4 show persistent rows have *identical per-warp SIMD-efficiency* to their non-persistent twins. It also adds live state to a kernel already at 92 % register residency (the EXP3 foreclosure pattern) and forfeits the `gid.x` adjacency the §6 ray-binning ~1.2× lives on. **(b) Occupancy** is register-capped (zero `var<workgroup>`), and the cap — the ~19-value active-`Frame` peak inside `make_frame` on descend — is the same one EXP1/EXP2 and Door B failed to move (re-captured residency unchanged); and it is *type-mismatched* — more resident warps cannot hide a 143-vs-1 serialization that happens *within* a warp. **(c) Branchless `walker_step`** targets ~3-ALU branch bodies on a 3 %-ALU kernel, cannot change the loop trip count, and adds unconditional `select` ALU (the EXP2 regression mechanism) at real byte-identical risk. **(d) The `@workgroup_size` sweep (EXP4)** was *built and measured* — `{32, 128, 256}` vs `64`, all 0/20000 — and is a **null**: 64 is already best (`dust` 2048³ worst-orientation gpu ns/ray ≈ 43–44 at 64 vs 45–47 at 32/128, ~44 at 256), confirming occupancy is register- not workgroup-capped. The *one* lever that would reach the 90 % — **wavefront compaction** (replace terminated rays mid-warp, Aila & Laine §5.1) — needs a subgroup prefix-sum / shuffle that WGSL/wgpu 29 does not expose, and was **net-null even for Aila & Laine on real hardware** (the ~30-instr/ray prefix-sum overhead ate it).

So the traversal floor for this kernel on Apple GPUs is **intrinsic intra-warp lockstep divergence** — Aila & Laine's "inherent challenge rather than a fully solvable problem." Every `> 1.4×` win in the literature (hardware SER 40–90 %, treelet prefetch ~90 % BW cut) requires a hardware primitive WGSL/wgpu does not expose. The kernel-micro-optimization line is therefore **comprehensively and empirically closed**: footprint (EXP1–3, null-to-negative), workgroup (EXP4, null), scheduling (persistent threads, foreclosed), register pressure (Door B + EXP1/2, falsified). The remaining paths are **not kernel levers** — they are to **budget around** the divergence (temporal reprojection / adaptive internal-resolution, §9.6), or to **re-express on the hardware ray-tracing pipeline** (Metal RT / shader-execution-reordering, §9.4), which is a different project, not a tweak.
