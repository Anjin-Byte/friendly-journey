// GPU occupancy generator for the procedural noise fixtures (`NoiseField`).
//
// A direct f32 port of `voxel-core`'s `noise.rs` (Perlin improved-noise gradient
// → fBm/ridged `fractal` → `domain_warp`) plus `NoiseField::value`. The CPU field
// stays the f64 reference/oracle; this is the *fast path* — noise evaluation is
// pure, per-voxel, divergence-free ALU work, exactly what the GPU is for (the CPU
// build saturates all cores yet still takes minutes at high resolution).
//
// One invocation per output `u32` word: it evaluates the 32 voxels of that word
// (linear index `x + y·n + z·n²`, the BitGrid bit order) and packs them. No
// atomics — every word has a single writer. f32 will not bit-match the f64
// reference at threshold-grazing voxels, but the field is statistically and
// visually identical; the differential/oracle path keeps using the CPU field.
...
fn generate(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let n = params.n;
    let nn = n * n; // n² (u32-safe for the resolutions this path serves)
    var word = 0u;
    for (var j = 0u; j < 32u; j = j + 1u) {
        let i = word_idx * 32u + j; // linear voxel index (< n³ ≤ u32::MAX, guarded on the host)
        let x = i % n;
        let y = (i / n) % n;
        let z = i / nn;
        if (occupied(x, y, z)) {
